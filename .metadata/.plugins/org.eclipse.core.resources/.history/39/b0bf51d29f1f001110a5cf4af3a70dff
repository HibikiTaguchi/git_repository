...
public class Practice0802Controller {
	...
	public String getItemListGroupByType(Model model) {
		model.addAttribute("typeList", typeRepo.findAll());
		return "practice08/02/search_input";
	}
	
	@GetMapping("/fruits_shop/named/result")
	public String searchResultFromInput(Integer minPrice,
			 Integer maxPrice, Integer typeId, Model model) {
		Type type = new Type();
		type.setTypeId(typeId);
		Query query = manager.createNamedQuery("findNamedQueryByPriceRangeAndType");
		query.setParameter("min", minPrice);
		query.setParameter("max", maxPrice);
		query.setParameter("type", type);
		model.addAttribute("resultItemList", query.getResultList());
